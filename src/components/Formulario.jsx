...
function Formulario({ pacientes, setPacientes, paciente, setPaciente }) {
  {/*LOS HOOKS SIEMPRE ANTES DEL CONTENIDO DEL COMPONENTE*/}
  ...
  const handleSubmit = (e) => {
    ...
    if([nombre, propietario, email, fecha, sintomas].includes('')){ //valida que no esten vacios
    ...
    setError(false); // regresar a false para que desaparezca el error
    // objeto de paciente
    ...
    if( paciente.id ){
      ...
      //identificar por id el objeto para modificar  
      const pacientesNuevos = pacientes.map( pacienteState => pacienteState.id === paciente.id ? objetoPaciente : pacienteState);
    ...
    <div className="md:w-1/2 lg:w-2/5">
        ...
        <form onSubmit={handleSubmit} className="bg-white shadow-md rounded-lg py-10 px-5 mb-10 mx-5">
          ...
          <div className="mb-5">
            ...
            <input id="email" type="email"  placeholder="Contacto del propietario" className="border-2 w-full p-2 mt-2 placeholder-gray-400 rounded-md"
          ...
          <input type="submit"  value={ paciente.id ? 'Editar Paciente' : 'Agregar Paciente' } className="bg-green-600 w-full p-3 text-white uppercase font-bold hover:bg-green-700 cursor-pointer transition-colors" />
        </form>
    </div>
  )
}
